...
public class commandgymcorrer implements CommandExecutor {
    private Plugin plugin;
    private Map<Player, Long> cooldowns;
    public commandgymcorrer(Plugin plugin) {
        this.plugin = plugin;
    ...
    public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
        ...
        String TotalVelocidad = "Mensaje-5";
        String RegionVelocidad = "Region-Velocidad";
        ...
        ProtectedRegion region = regionManager.getRegion(ChatColor.translateAlternateColorCodes('&', plugin.getConfig().getString(RegionVelocidad)));
        ...
        new BukkitRunnable() {
            ...
            public void run() {
                ...
                double currentStrengthValue = player.getAttribute(Attribute.GENERIC_MOVEMENT_SPEED).getBaseValue();
                double newStrengthValue = currentStrengthValue + 0.1;
                player.getAttribute(Attribute.GENERIC_MOVEMENT_SPEED).setBaseValue(newStrengthValue);
                ...
                double currentStrength = playerSection.getDouble("velocidad", 0.0);

                // Actualizar la fuerza y guardarla en la sección del jugador
                double newStrength = currentStrength + 0.1;
                playerSection.set("velocidad", newStrength);

                // Guardar la configuración en el archivo
                plugin.saveConfig();

                player.sendMessage(ChatColor.translateAlternateColorCodes('&', plugin.getConfig().getString(TotalVelocidad).replace("%velocidadnueva%",String.valueOf(newStrength))));
